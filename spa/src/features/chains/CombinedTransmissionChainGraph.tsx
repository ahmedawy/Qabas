...
// Custom CustomNode component matching our glassmorphic design system
...
const getLayoutedElements = (
  ...
  // Set layout options: direction and separation
  dagreGraph.setGraph({ rankdir: direction, nodesep: 40, ranksep: 100 });

  nodes.forEach((node) => {
    // Width and height of our custom card
...
export const CombinedTransmissionChainGraph: React.FC<CombinedTransmissionChainGraphProps> = ({
  sanadIds,
  onSelectNarrator,
}) => {
  ...
  const fetchCombinedGraph = useCallback(async () => {
    ...
    try {
      const res = await api.getCombinedTransmissionChain(sanadIds);
  ...
  }, [sanadIds, onSelectNarrator, setNodes, setEdges]);
  ...
  if (sanadIds.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-slate-800 py-12 text-center text-slate-500 text-sm">
        الرجاء تحديد أسانيد من علامة تبويب "طرق وأسانيد الرواية" للرسم المجمع.
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-950/20 border border-red-900/50 rounded-xl text-red-400 text-sm text-center">
        فشل رسم شجرة الإسناد المجمعة: {error}
      </div>
    );
  }
    ...
    <div className="space-y-4" dir="ltr">
      <div className="flex justify-between items-center" dir="rtl">
        <div className="text-xs text-slate-400">
          مخطط مجمع لـ {sanadIds.length} أسانيد • {nodes.length} رواة
        </div>
        <button
