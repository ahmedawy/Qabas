...
class GetLexiconPlacesController extends Controller
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        // Case 1: Return definitions/descriptions for a place
        if ($itemId !== null) {
            $id = (int) $itemId;
        ...
        // Case 3: Browse children (default parent_id to 1 for Places)
        $pid = $parentId !== null ? (int) $parentId : 1;
        $items = $this->lexiconItemModel->newQuery()
            ->where('LexiconID', 3)
            ->where('ParentID', $pid)
            ->orderBy('ID')
